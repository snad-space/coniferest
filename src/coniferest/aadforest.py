...
class AADEvaluator(ConiferestEvaluator):
    ...
    def fit_known(self, data, known_data, known_labels, prior_weights=None):
        """
        Fit the model using known labeled instances.

        The method solves the optimization problem:

        .. math::
            \vb w = \arg\min_{\vb w} \left(
            \frac{C_a}{\left|\mathcal{A}\right|}
                \sum_{i \in \mathcal{A}} \mathrm{ReLU}\left(s(\vb x_i | \vb w) - q_{\tau}\right)\right.\\
            \left.
            \frac{1}{\left|\mathcal{N}\right|}
                \sum_{i \in \mathcal{N}} \mathrm{ReLU}\left(q_{\tau} - s(\vb x_i | \vb w)\right) +
            \frac{1}{2} \lVert \vb w - \vb w_0 \rVert^2\right)

        This problem is reformulated as an equivalent quadratic programming problem
        with convex constraints by introducing additional auxiliary variables $\vb u$:

        .. math::
            \begin{bmatrix}
            \vb w\\
            \vb u
            \end{bmatrix} = \arg\min_{\vb w, \vb u} \left(
        ...
        .. math::
            ...
            u_i - s(\vb x_i | \vb w) \ge - q_{\tau}\quad i \in \mathcal{A},\\
            u_i + s(\vb x_i | \vb w) \ge q_{\tau}\quad i \in \mathcal{N}.\\

        Parameters
        ----------
        data
            Unlabeled features for quantile estimation.
        known_data
            Labeled instances with known anomaly/regular labels.
        known_labels
            Labels for known_data (ANOMALY or REGULAR).
        prior_weights
            Initial weights to use instead of self.weights. If None, uses current weights.
